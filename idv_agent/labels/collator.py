...
class FastControllerCollator:
    ...
    def __call__(self, batch: list[dict]) -> dict:
        ...
        intent_vector = torch.as_tensor([b["intent_vector"] for b in batch], dtype=torch.float32)
        category_labels = torch.as_tensor([b["category_id"] for b in batch], dtype=torch.long)
        continuous_labels = torch.as_tensor([b["numeric_action"] for b in batch], dtype=torch.float32)
