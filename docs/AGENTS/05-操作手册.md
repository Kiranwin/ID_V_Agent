...
所有命令以项目根目录为当前目录；当前唯一运行架构是 SGan（原 M29，见 [agent.md](../run-agent.md)）。
...
与视角转动一致、标注端点切在自己要监督的那一帧。详见 [tools/replay-tool.md](../replay-tool.md)。
...
见 [tools/data-workbench-v6.md](../data-workbench-v6.md)。
...
`--send-input --allow-undeployed-send-input`。字段、依赖脚本、trace 结构与故障排查见 [agent.md](../run-agent.md)。
